...
_example_EPER_data = np.array(_example_EPER_data) + _gen.normal(loc=0.0, scale=0.2, size=20)
_siglevel = 10000.0
...
def test_single_EPER_fit_subtraction():
    ftr = SingleEPERTrailFitter(N_transfers=1024, do_trapfit=False, subtract_eper_zeros=10)
...
def sample_PTC_table_EPER_data():
    tab = pd.DataFrame({"meablorp": [_siglevel], "speeeepertrail" : [_example_EPER_data],
...
def test_PTC_EPER_fit(sample_PTC_table_EPER_data):
    ...
    ftr = PTCEPERFitter(pass_columns = [], siglevelcol="meablorp", ser_eper_col="speeeepertrail",
                        llel_eper_col="badaboopeper", ser_settings=settings, llel_settings=settings)
    res = ftr.run(ptcres)
...
def test_PTC_EPER_fit_selector(sample_PTC_table_EPER_data):
    ...
    ftr = PTCEPERFitter(pass_columns = [], siglevelcol="meablorp", ser_eper_col=ser_eper_select,
                        llel_eper_col="badaboopeper", ser_settings=settings, llel_settings=settings)
