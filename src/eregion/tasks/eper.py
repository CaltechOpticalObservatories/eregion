...
class EPERFitUncalibratedResult(TaskResult):
    ...
    trap_rates: list[float] | None = Field(
        description="fitted trap decay rates", default=None
    )
    trap_amplitudes: list[float] | None = Field(
        description="fitted trap amplitudes", default=None
    )
    eper_baseline: Number | None = Field(
        description="EPER trail baseline subtracted", default=None
    )
...
class SingleEPERTrailFitter(Task):
    ...
    def __init__(
        self,
        N_transfers: int,
        do_trapfit: bool = True,
        N_trap_species: Optional[int] = None,
        decay_rate_tolerance: float = 0.25,
        name: Optional[str] = None,
        subtract_eper_zeros: Optional[int] = None,
        **kwargs,
    ):
    ...
    def run(self, siglevel: float, eper_trail: np.ndarray) -> EPERFitUncalibratedResult:
        ...
        if self.do_trapfit:
            ...
            self.logger.debug(
                f"thetas: {efitter.thetas}, k: {efitter.ks}, a: {efitter.a}"
            )
...
class PTCEPERFitter(LazyTask):
    ...
    def __init__(
        self,
        siglevelcol: _Extractor,
        ser_eper_col: _Extractor,
        ...
        ftrs = {"ser": ser_settings, "llel": llel_settings}
        ...
        for ftrname, ftr in ftrs.items():
            match ftr:
                ...
                case _:
                    raise TypeError(
                        f"type of supplied argument for {ftrname} fitter is invalid."
                    )
    ...
    def lazy_run(self, ptc_results: PTCResult) -> Generator[PTCEPERFitResult]:
        ...
        for row in ptc_results.ptc_table.itertuples(index=False):
            ...
            def do_EPER_analysis(EPER_trail_col, fitter, direction):
                self.logger.info(
                    "running %s EPER fit on row with id %s and signal level %s",
                    direction,
                    ident,
                    siglevel,
                )
