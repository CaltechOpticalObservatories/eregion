...
_TDISTATS: dict[str, Callable] = {
    k: STATFUNCS[k] for k in ["mean", "median", "std", "mad"]
}
...
class TDIExtractPTC(LazyTask):
    ...
    def lazy_run(
        self, images: ImageBundle | Iterable[DetImage]
    ) -> Generator[PTCResult, None, None]:
    ...
    def _process_group(
        ...
        for i, img in enumerate(images):
            self.logger.debug(
                "Doing per-output stats on individual images in group, %d of %d",
                i,
                len(images),
            )
        ...
        for dpidx0, dpidx1 in combinations(range(len(images)), 2):
            self.logger.debug(
                "doing diffpair calculation on images %d and %d", dpidx0, dpidx1
            )
