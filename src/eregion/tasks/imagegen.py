...
class ImageResult(TaskResult):
        ...
        for key, val in kwargs.items():
            ...
            if isinstance(val, list):
                if all(isinstance(x, DetImage) for x in val):
                    ...
                    raise ValueError(
                        f"Invalid input. Value of {key} must be an ImageBundle, FPImageBundle, or a list "
    ...
    def save(self, filepath: str, **kwargs) -> None:
        for attr, value in self.payload_dict().items():
            if isinstance(value, ImageBundle):
                if len(value) == 0:
                    warnings.warn(
                        f"ImageBundle for attribute {attr} is empty. Not saving."
                    )
                else:
    ...
    def load(cls, filepath: str):
        ...
        for attr in cls.payload_field_names():
            if isinstance(
                cls.model_fields[attr].annotation, type(ImageBundle | FPImageBundle)
            ):
...
class ImageCreator(LazyTask):
    ...
    def set_identifier(self, func: str | Callable, **kwargs) -> None:
        ...
        if (
            "filename" not in sig.parameters.keys()
            and "headers" not in sig.parameters.keys()
        ):
            raise ValueError(
                "Identifier function must have either 'filename' or 'headers' as an argument"
            )
    ...
    def _build_image_objects(self, input_data, data_on_demand=False) -> list[Any]:
        ...
        if isinstance(input_data, str):
            ...
            if data_on_demand:
                ...
                input_data_array = []
                self.logger.debug(
                    "Data will be loaded on demand for file %s", input_data
                )
            else:
                self.logger.debug(
                    "Loaded %d HDU from file %s", len(input_data_array), input_data
                )
            ...
            self.logger.debug(
                "Data provided as array input, got %d HDU worth of headers",
                len(input_headers),
            )
        else:
    ...
    def _build_single_image_object(
        self,
        obj,
        output_class: str,
        input_data_array,
        ...
        if filename is not None:
            if not glob2.fnmatch.fnmatch(
                os.path.basename(filename), obj["filename_format"]
            ):
                self.logger.info(
                    "Skipping file %s as it does not match filename format for this object (%s)",
                    filename,
                    obj["filename_format"],
                )
                return None
        ...
        self.logger.info(
            "Building object %s with %s %s outputs from file %s",
            image_class,
            len(outputs),
            output_class,
            filename or "array input",
        )

        prihdr = (
            input_headers[obj["header_index"]]
            if len(input_headers) > 0 and "header_index" in obj
            else dict()
        )
        ...
        if not image.validate_size_from_outputs():
            raise ValueError(
                "inconsistent image sizes from output objects and metadata"
            )
        ...
        if len(input_data_array) > 0:
            ...
            for output_id, output_obj in image.outputs.items():
                hdr, dat = output_obj._select_header_and_data(
                    input_headers, input_data_array
                )
                output_obj.header = hdr
        ...
        **kwargs,
        ...
        match input_source:
            ...
            case _:
                ...
                raise TypeError(
                    "Input source must be either a path to FITS files or an iterable of numpy arrays"
                )
        ...
        for input_batch in input_batches:
            ...
            for batch in batched(input_batch, self.max_batch_size):
                ...
                for inp in batch:
                    ...
                    # Build image objects
                    images = self._build_image_objects(
    ...
    def run(
        ...
        **kwargs,
        ...
        if from_path is not None:
            ...
            self.bundle = (
                from_images
                if isinstance(from_images, ImageBundle)
                else ImageBundle(from_images)
            )
        else:
            raise ValueError(
                "Must provide either from_path or from_images to assemble focal plane images."
            )
        ...
        for unique_keys, group in self.bundle.groupby(by=groupby_keys):
            if not (len(group) <= self.num_detectors and group["det_id"].is_unique):
                self.logger.warning(
                    f"Group with groupby column values {unique_keys} has incorrect number of detectors."
